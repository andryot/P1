...
class Luna:
    def __init__(self):
        self.odvisniki = OrderedDict()
        self.vsi = list(self.lune())


    def pripni(self, ime):
        self.odvisniki[len(self.odvisniki)] = ime


    def lune(self):
        return set(self.odvisniki.values())


    def ima_luno(self, ime):
        return ime in self.odvisniki.values()


    def stevilo_lun(self):
        return len(self.odvisniki)


    def ima_odvisnika(self, x):
        self.vsi = list(self.lune())
        for odvisnik in self.vsi:
            if isinstance(odvisnik,Luna):
                self.vsi.extend(odvisnik.lune())
        return x in self.vsi
    ...
    def sirina_orbite(self, n):
        if n == 0:
            return 1
        sirina = 0
        for luna in self.odvisniki.values():
...
class EkskluzivnaLuna(Luna):
    def pripni(self, ime):
        if len(self.odvisniki) < 3:
            super().pripni(ime)
        else:
            self.odvisniki[0], self.odvisniki[1], self.odvisniki[2] = self.odvisniki[1], self.odvisniki[2], ime
